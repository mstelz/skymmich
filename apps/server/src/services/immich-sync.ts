...
// Plate solving machine tags include individual star names which are too noisy for Immich.
// Only keep catalog objects (M/NGC/IC/Abell/Sh2/LDN/LBN), user-created tags, and known categories.
const CATALOG_PATTERN = /^(M\s?\d|NGC\s?\d|IC\s?\d|Abell\s?\d|Sh2-|LDN\s?\d|LBN\s?\d|Barnard|Cr\s?\d|Mel\s?\d|PGC\s?\d|UGC\s?\d|Ced\s?\d|vdB\s?\d)/i;
const STAR_PATTERN = /^The star\b/i;
